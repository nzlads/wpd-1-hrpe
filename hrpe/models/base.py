...
class PeriodModel:
    expected_cols = ["time", "value_max", "value_min", "value_mean", "delta_max", "delta_min", "period"]
    ...
    def check_train_data(self, train: pd.DataFrame):
        ...
        expected_dates = pd.date_range(start=train["time"].min(), end=train["time"].max(), freq="30T")
        assert len(expected_dates) == len(train), "Some dates are missing"
        assert train["time"].is_monotonic_increasing, "Dates are not strictly increasing"

        period_counts = train["period"].value_counts()
        assert len(period_counts) == 48, "There should be 48 periods"
        assert len(period_counts.unique()) == 1, "The periods in the training data should be the same length"
