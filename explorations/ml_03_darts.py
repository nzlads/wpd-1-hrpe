...
from hrpe.data.load import filter_data_by_time, load_minute_data, load_hh_data, load_maxmin_data
...
hh_valid = filter_data_by_time(hh_data, time_start=TRAIN_DATE, time_end=None).reset_index(drop=True)

# Use darts class
dmax_ts = TimeSeries.from_dataframe(hh_train, time_col="time", value_cols="delta_max", freq="30min")
dmin_ts = TimeSeries.from_dataframe(hh_train, time_col="time", value_cols="delta_min", freq="30min")

valid_dmax_ts = TimeSeries.from_dataframe(hh_valid, time_col="time", value_cols="delta_max", freq="30min")
valid_dmin_ts = TimeSeries.from_dataframe(hh_valid, time_col="time", value_cols="delta_min", freq="30min")
...
# Now let's predict on everything
