...
from hrpe.features.weather import prep_weather
...
weather_data = prep_weather(weather_data)
...
def fit_score_delta_model(train, test, covs, data, target):
    ...
    model = DeltaLGBMModel(
        target=target,
        covariates=covs,
        lags={"lags": None, "lags_future_covariates": [0]},
    )
...
n_jobs = cpu_count()
parallel = Parallel(n_jobs=n_jobs, verbose=7)
min_scores = parallel(
